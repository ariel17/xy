...
func TestDeviceRegistration(t *testing.T) {
	...
	t.Run("Getting the user's devices", func(t *testing.T) {
		t.Run("Existing user", func(t *testing.T) {
			...
			user := domain.NewUser("ariel17")
			dao.Client.InsertUser(user)

			// device := domain.NewDevice(user.ID)

			req, _ := http.NewRequest("GET", fmt.Sprintf("/users/%s/devices", string(user.ID)), nil)
			resp := httptest.NewRecorder()
			...
			expectedBody := `{"status":"success","message":"User's devices fetch successful.","data": [{"_id":"1","model":"Model X","created_at":"2018-01-01T00:00:00Z"},{"_id":"2","model":"Model Y","created_at":"2018-01-02T00:00:00Z"}]}`
			if body := strings.TrimSpace(resp.Body.String()); body != expectedBody {
				t.Errorf("body mismatch! expected %s, got %s", expectedBody, body)
				t.FailNow()
			}
		...
		t.Run("User not found", func(t *testing.T) {
			req, _ := http.NewRequest("GET", "/users/99999/devices", nil)
			resp := httptest.NewRecorder()
			...
			expectedBody := `{"status":"failed","message":"User not exists."}`
			if body := strings.TrimSpace(resp.Body.String()); body != expectedBody {
				t.Errorf("body mismatch! expected %s, got %s", expectedBody, body)
				t.FailNow()
			}
